...
function ManagerForm({ project, employees, close, reload }) { return <FormModal title="Change project manager" close={close} label="Save manager" onSubmit={async values => {
  ...
  options={[["", 'Choose an employee…'], ...employees.filter(employee => ['Active', 'On leave'].includes(employee.status) || employee.id === project.managerEmployeeId)
    .map(employee => [employee.id, `${employee.name} — ${employee.designation}`])]} /></FormModal>; }
