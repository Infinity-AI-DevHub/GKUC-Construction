...
router.get('/:id', auth, wrap(async (req, res) => {
  ...
  const [milestones, documents, team, tasks, expenses, incomes, boqs] = await Promise.all([
    ...
    query(`SELECT d.id,d.title,d.category,d.file_ref fileRef,d.created_at createdAt,u.name uploadedBy
      FROM project_documents d JOIN users u ON u.id=d.uploaded_by WHERE d.project_id=? ORDER BY d.id DESC`, [project.id]),
    ...
    query('SELECT COALESCE(SUM(amount),0) total FROM expenses WHERE project_id=?', [project.id]),
...
/* Project documents and team (PID 2.4) */
router.post('/:id/documents', auth, permit(roles.projects), validate(z.object({
  title: z.string().min(2).max(180),
  category: z.string().min(2).max(80),
  fileRef: z.string().min(2).max(400)
})), wrap(async (req, res) => {
  const result = await query('INSERT INTO project_documents (project_id,title,category,file_ref,uploaded_by) VALUES (?,?,?,?,?)',
    [req.params.id, req.body.title, req.body.category, req.body.fileRef, req.user.id]);
  const row = await getOne('SELECT * FROM project_documents WHERE id=?', [result.insertId]);
  await audit(pool, req.user.id, 'CREATE', 'project_document', row.id, null, row, req.ip);
  res.status(201).json(row);
}));
